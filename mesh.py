...
def create_duplicate_and_groups(source_obj,suffix, broken = False):
    """
    ...
    if broken:
        ...
        new_name = f"{base_name}_{suffix}_broken"
    else:
        bpy.context.collection.objects.link(new_obj)
...
def bone_duplicate_and_anim(suffix, new_obj, broken= False):
    # Создаем кость
    if broken:
...
def create_mat_and_shaders(broken = False):
    # Название для карты
    if broken:
        mat_name = f"{base_name_planet}_{race}_broken"
...
for suffix, race in zip(suffixes, image_race_list):
    # Меш и группы
    new_obj = create_duplicate_and_groups(source_obj, suffix)
    new_obj_broken = create_duplicate_and_groups(source_obj, suffix, True)
    # Кость и анимация
    new_bone = bone_duplicate_and_anim(suffix, new_obj)
    new_bone_broken = bone_duplicate_and_anim(suffix, new_obj_broken, True)
    # Материал и Шейдеры
